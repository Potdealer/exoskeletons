...
function initProvider() {
  ...
  contracts = {
    ...
    marketplace: new ethers.Contract(CONTRACTS.marketplace, MARKETPLACE_ABI, provider),
  };
}
...
async function withFallback(fn) {
  try {
    ...
    if (rpcIndex < RPC_URLS.length - 1) {
      ...
      contracts.marketplace = new ethers.Contract(CONTRACTS.marketplace, MARKETPLACE_ABI, provider);
      return await fn();
    }
    throw e;
  }
...
function getSignedContract(contractName) {
  ...
  const abis = { core: CORE_ABI, registry: REGISTRY_ABI, renderer: RENDERER_ABI, wallet: WALLET_ABI, marketplace: MARKETPLACE_ABI };
...
// ── Marketplace Helpers ──
let _marketplaceIface = null;
function getMarketplaceIface() {
  if (!_marketplaceIface) _marketplaceIface = new ethers.Interface(MARKETPLACE_ABI);
  return _marketplaceIface;
}

async function loadMarketplaceStats() {
  initProvider();
  const stats = await withFallback(() => contracts.marketplace.getStats());
  return {
    totalModules: Number(stats.totalModules),
    totalApproved: Number(stats.totalApproved),
    totalActivations: Number(stats.totalActivations),
    totalPlatformRevenue: stats.totalPlatformRevenue,
    pendingCount: Number(stats.pendingCount),
    listingFees: stats.listingFees,
  };
}

async function loadAllMarketplaceModules() {
  initProvider();
  const names = await withFallback(() => contracts.marketplace.getAllModuleNames());
  if (names.length === 0) return [];

  const mc = new ethers.Contract(MULTICALL3, MULTICALL_ABI, provider);
  const mIface = getMarketplaceIface();
  const calls = names.map(n => ({
    target: CONTRACTS.marketplace,
    callData: mIface.encodeFunctionData('getModule', [n]),
  }));
  ...
  for (let i = 0; i < names.length; i++) {
    if (results[i].success) {
      try {
        const d = mIface.decodeFunctionResult('getModule', results[i].returnData);
        modules.push({
          nameHash: names[i],
          builder: d.builder,
          name: d.name,
          ...
          approvedAt: Number(d.approvedAt),
          activations: Number(d.moduleActivations),
          revenue: d.moduleRevenue,
        });
      } catch (e) {}
    }
  }
...
async function loadBuilderProfile(address) {
  initProvider();
  const b = await withFallback(() => contracts.marketplace.getBuilder(address));
  return {
    name: b.name,
    bio: b.bio,
    modulesSubmitted: Number(b.modulesSubmitted),
